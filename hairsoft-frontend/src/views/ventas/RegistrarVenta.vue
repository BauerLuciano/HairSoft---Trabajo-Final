<template>
  <div class="venta-page">
    ...
    <transition name="toast">
      ...
      <PagoQrModal
        v-if="qrFullscreen && mpQrData"
        :init-point="mpQrData.init_point"
        :monto="montoQRMostrar"
        :monto-label="esMixto ? 'Monto a transferir' : (pagoParcialMP ? 'Saldo pendiente a transferir' : 'Total a pagar')"
        :monto-extra="esMixto ? `Resto $${restanteEfectivo.toFixed(2)} en efectivo` : (pagoParcialMP ? `Total $${total.toFixed(2)} − ya abonado $${pagoParcialMP.toFixed(2)}` : '')"
        :monto-aviso="pagoParcialMP ? 'Este QR cobra únicamente el saldo pendiente' : ''"
        :estado="mpPagoEstado || 'pending'"
        boton-continuar="Confirmar venta"
        :boton-deshabilitado="procesandoVenta"
        status-ok-sub="Registrando la venta automáticamente..."
        @cancelar="regresarDelQR"
        @continuar="manejarBotonFullscreen"
      />
  </div>
</template>
...
export default {
    ...
    components: { AsignarEnvio, QrcodeVue, PagoQrModal },
