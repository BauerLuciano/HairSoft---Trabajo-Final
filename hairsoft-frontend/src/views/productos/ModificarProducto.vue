<template>
  <div class="pedido-container">
    ...
    <div v-if="cargandoProducto" class="loading-state">
      <div class="spinner"></div>
      <p>Cargando datos...</p>
    </div>

    <form v-else @submit.prevent="modificarProducto">
          ...
          <div class="image-upload-wrapper">
            ...
            <div class="image-preview-container" @click="$refs.fileInput.click()">
              <div v-if="imagenPreview" class="preview-active">
                ...
                <button @click.stop="quitarImagen" class="btn-remove-img" type="button" title="Quitar imagen">
                  <X :size="16" />
                </button>
              </div>
...
import { 
  ...
  Barcode, FileText, DollarSign, Truck, Check, CheckCircle2, 
  Loader2, AlertTriangle, Camera, UploadCloud, X 
...
const proveedoresActivos = computed(() => proveedores.value.filter(p => p.estado === 'ACTIVO' || p.activo === true))
...
const cargarProducto = async () => {
  ...
  try {
    ...
    producto.value = {
      ...
      lote_reposicion: data.lote_reposicion || 1,
    ...
    if (data.imagen) {
        const imgUrl = data.imagen.startsWith('http') 
            ? data.imagen 
            : `http://127.0.0.1:8000${data.imagen}`
        imagenPreview.value = imgUrl
    }
...
const modificarProducto = async () => {
  ...
  try {
    ...
    formData.append('codigo', producto.value.codigo.toString().trim())
    formData.append('descripcion', producto.value.descripcion.trim())
    formData.append('precio', String(producto.value.precio))
    formData.append('stock_actual', Number(producto.value.stock_actual))
    formData.append('stock_minimo', Number(producto.value.stock_minimo))
    formData.append('lote_reposicion', Number(producto.value.lote_reposicion))
    ...
    const res = await axios.put(`/usuarios/api/productos/${props.productoId}/`, formData, config)
...
<style scoped>
/* ESTILOS PREMIUM UNIFICADOS (IDÉNTICOS A REGISTRAR) */
...
.proveedores-grid { display: grid; grid-template-columns: repeat(auto-fit, minmax(240px, 1fr)); gap: 15px; margin-bottom: 15px; width: 100%; }
...
@media (max-width: 768px) {
  ...
  .proveedores-grid { grid-template-columns: 1fr; }
}
</style>